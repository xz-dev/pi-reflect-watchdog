...
Raw failing evidence for the defective candidate is saved at `/tmp/pi-watchdog-controls-repair-red.txt`. It demonstrates the review findings before the repair: Pi command wrappers around the same session were rejected, and slash-limit crossings sent a model message.
